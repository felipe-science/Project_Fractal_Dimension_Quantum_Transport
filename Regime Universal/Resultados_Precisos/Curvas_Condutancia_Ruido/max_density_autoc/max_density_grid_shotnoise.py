...
# Listar arquivos no diretório atual que começam com "SN"
files = [f for f in os.listdir() if f.startswith('SN') and f.endswith('.dat')]
...
# Criar lista para armazenar os dados para o arquivo
output_data = []
ro_values = []  # Para armazenar valores de ro

# Função para processar e plotar dados
def process_and_plot(ax, filename, label, m_value):
    ...
    autc = autocorrelation(y)

    target_value = 0.5

    # Encontrar a posição do valor mais próximo de 0.5
    position_closest = (np.abs(autc - target_value)).argmin()

    ro = len(listx_max) / 0.2
    tau_ro = 1 / (6 * ro)

    # Armazenar o valor de ro e o nome do arquivo
    ro_values.append((filename, ro))

    # Remover símbolos especiais do LaTeX
    beta_label = label.replace(r'$\beta = ', 'beta').replace('$', '').strip()
    
    # Armazenar os dados para o arquivo
    output_data.append(f"{beta_label}  m{m_value} ruido      tau_ro = {tau_ro}     rho={ro}")

    # Plotar os dados no subplot específico
    ax.plot(x, y, color='green')
    ax.scatter(listx_max, listy_max, color='red')
    ax.set_xlabel(r"Energy $(eV)$", fontsize=24)
    ax.set_ylabel(r"Shot Noise Power", fontsize=24)
    ...
    ax.text(0.05, 0.84, f'{label}\nm = {m_value}', transform=ax.transAxes, fontsize=14, color='black',
            bbox=dict(facecolor='white', alpha=0.8))

# Processar e plotar gráficos para os arquivos m3
if files_beta1_m3:
    process_and_plot(axs[0, 0], files_beta1_m3[0], r'$\beta = 1$', '3')
if files_beta2_m3:
    process_and_plot(axs[1, 0], files_beta2_m3[0], r'$\beta = 2$', '3')
if files_beta4_m3:
    process_and_plot(axs[2, 0], files_beta4_m3[0], r'$\beta = 4$', '3')

# Processar e plotar gráficos para os arquivos m4
if files_beta1_m4:
    process_and_plot(axs[0, 1], files_beta1_m4[0], r'$\beta = 1$', '4')
if files_beta2_m4:
    process_and_plot(axs[1, 1], files_beta2_m4[0], r'$\beta = 2$', '4')
if files_beta4_m4:
    process_and_plot(axs[2, 1], files_beta4_m4[0], r'$\beta = 4$', '4')

# Ajustar layout
plt.tight_layout()

# Salvar o gráfico com os subplots posicionados verticalmente
plt.savefig('shot_noise.png', dpi=500)
plt.show()

# Salvar os dados em um arquivo
with open('conductance_shot.txt', 'w') as f:
    for line in output_data:
        f.write(line + '\n')

# Imprimir o nome do diretório e o valor de ro
print("Nome do diretório\tValor de ro")
for filename, ro in ro_values:
    print(f"{filename}\t{ro}")
