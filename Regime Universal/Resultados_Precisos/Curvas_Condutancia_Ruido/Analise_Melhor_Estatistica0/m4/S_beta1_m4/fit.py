...
def f(delta_epsilon, g, a, b, c):
    # Evitar divisão por zero
    g = max(g, 1e-10)  # Define um valor mínimo para Gamma
    return 1 / (1 + (delta_epsilon / g)**(2 * b))**a + c


def density_ro(g, a, b, c):
...
initial_guess = [1, 1, 1, 1]  # Gama, alpha, beta iniciais

# Definir limites para evitar valores inválidos
popt, _ = curve_fit(f, datax, datay, p0=initial_guess, bounds=([1e-10, 0, 0, -np.inf], [np.inf, np.inf, np.inf, np.inf]))
Gamma_fit, alpha_fit, beta_fit, c_fit = popt
...
ro = density_ro(Gamma_fit, alpha_fit, beta_fit, c_fit)
...
y_curve = []
for x in x_curve:
    y = f(x, Gamma_fit, alpha_fit, beta_fit, c_fit)
    y_curve.append(y)
