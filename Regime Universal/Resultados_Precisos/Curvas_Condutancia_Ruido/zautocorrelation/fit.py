...
def process_directory(directory):
    ...
    initial_guess = [1, 1, 1]  # Adicionando o parâmetro beta
    try:
        # Definir limites para evitar valores inválidos
        popt, _ = curve_fit(
            lorentziana_generalizada, x_data, data, p0=initial_guess, 
            bounds=([1e-10, 0, 0], [np.inf, np.inf, np.inf])
        )
        Gamma_fit, alpha_fit, beta_fit = popt
        return x_data, data, popt, Gamma_fit, alpha_fit, beta_fit
    except (RuntimeError, ValueError) as e:
        print(f"Ajuste falhou para o diretório: {os.path.basename(directory)}. Erro: {str(e)}")
        return None, None, None, None
...
for idx, directory in enumerate(all_directories):
    x_data, data, popt, Gamma_fit, alpha_fit, beta_fit = process_directory(directory)
    ...
    if x_data is not None and data is not None and popt is not None:
        ...
        plt.xlim(0, 30)
        plt.title(f"{os.path.basename(directory)}: α = {alpha_fit:.4f}, Γ = {Gamma_fit:.4f}, β = {beta_fit:.4f}")
        plt.xlabel("Delta Epsilon")
        plt.ylabel("Autocorrelação")
        plt.legend()
        
        # Calcular o valor de ρ com a nova fórmula
        rho = (1 / (2 * np.pi)) * np.sqrt(6 * beta_fit * (alpha_fit + 1) / (Gamma_fit**2))
        
        # Armazenar os resultados
        results.append(f"{os.path.basename(directory)} {alpha_fit:.4f} {Gamma_fit:.4f} {beta_fit:.4f} {rho:.4f}")
...
# Salvar os resultados em output.dat
with open("output.dat", "w") as f:
    f.write("# Nome do Experimento, Alpha, Gamma, Beta, Rho\n")
