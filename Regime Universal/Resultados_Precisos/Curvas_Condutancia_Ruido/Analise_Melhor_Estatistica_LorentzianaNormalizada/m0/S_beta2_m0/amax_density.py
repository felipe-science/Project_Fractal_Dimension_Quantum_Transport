...
total_files = [f for f in os.listdir(directories) if f.startswith('S') and f.endswith('.dat')]
...
f1 = open("amax_point.dat", "w")
f2 = open("amax_curve.dat", "w")

N = len(datay)
for i in range(1,N-1,1):
    if(datay[i] > datay[i-1] and datay[i] > datay[i+1]):
        plt.scatter(datax[i], datay[i], color="red")
        f1.write(f"{datax[i]} {datay[i]}\n")
