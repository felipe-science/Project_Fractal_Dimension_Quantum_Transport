...
xc = G_m1_beta1_curve[:,0]/list_gamma_beta1[1]
yc = G_m1_beta1_curve[:,1]
xp = G_m1_beta1_point[:,0]/list_gamma_beta1[1]
yp = G_m1_beta1_point[:,1]
mean = np.average(yc)

axs[0].plot(xc, yc, label=r'$g(\epsilon)$', color='red', zorder=1)
axs[0].scatter(xp, yp, color='black', zorder=2)
axs[0].set_title(r'$m = 1$',  fontsize=35)
...
xc = G_m1_beta2_curve[:,0]/list_gamma_beta2[1]
yc = G_m1_beta2_curve[:,1]
xp = G_m1_beta2_point[:,0]/list_gamma_beta2[1]
yp = G_m1_beta2_point[:,1]
mean = np.average(yc)
...
xc = G_m1_beta4_curve[:,0]/list_gamma_beta4[1]
yc = G_m1_beta4_curve[:,1]
xp = G_m1_beta4_point[:,0]/list_gamma_beta4[1]
yp = G_m1_beta4_point[:,1]
mean = np.average(yc)
...
plt.savefig("total_amax_m1.png", dpi=200)
plt.show()