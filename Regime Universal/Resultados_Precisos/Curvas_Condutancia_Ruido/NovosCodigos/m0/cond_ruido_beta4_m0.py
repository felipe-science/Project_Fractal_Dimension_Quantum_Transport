...
def G_E_desordemfixa(syst, l):
    ...
    salt = np.random.randint(2000000) # Seed
    t = time.time()
    f = open("G_m"+str(Nf)+"_n"+str(L)+"w370_R"+str(l)+".dat","w")
    g = open("S_m"+str(Nf)+"_n"+str(L)+"w370_R"+str(l)+".dat","w")
    for E in energies:
        smatrix = kwant.smatrix(syst, energy=E, params=dict(salt=salt, W=W*sigma_0))
        G_10    = smatrix.transmission(1,0)
...
def main():
    ...
    for l in range(2):
        ...
        os.system(f"mv G_m3_n53w370_R{l}.dat ./G_m3_beta4")
        os.system(f"mv S_m3_n53w370_R{l}.dat ./S_m3_beta4")
