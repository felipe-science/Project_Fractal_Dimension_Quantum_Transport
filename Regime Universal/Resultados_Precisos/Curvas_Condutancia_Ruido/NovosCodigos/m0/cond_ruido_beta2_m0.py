...
def G_E_desordemfixa(syst, l):
    ...
    salt = np.random.randint(2000000) # Seed
    t = time.time()
    f = open("G_m"+str(Nf)+"_n"+str(L)+"w213_R"+str(l)+".dat","w")
    g = open("S_m"+str(Nf)+"_n"+str(L)+"w213_R"+str(l)+".dat","w")
    for E in energies:
...
def main():
    ...
    for l in range(2):
        ...
        os.system(f"mv G_m3_n53w213_R{l}.dat ./G_m3_beta2")
        os.system(f"mv S_m3_n53w213_R{l}.dat ./S_m3_beta2")
