...
def G_E_desordemfixa(syst, l):
    ...
    salt = np.random.randint(2000000) # Seed
    t = time.time()
    f = open("G_m"+str(Nf)+"_n"+str(L)+"w298_R"+str(l)+".dat","w")
    g = open("S_m"+str(Nf)+"_n"+str(L)+"w298_R"+str(l)+".dat","w")
    for E in energies:
...
def main():
    ...
    for l in range(0,400,1):
        ...
        shutil.move(f"mv G_m2_n53w298_R{l}.dat", "./G_m2_beta2")
        shutil.move(f"mv S_m2_n53w298_R{l}.dat", "./S_m2_beta2")
