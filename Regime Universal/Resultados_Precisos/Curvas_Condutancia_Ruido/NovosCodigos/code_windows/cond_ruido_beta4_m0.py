...
def G_E_desordemfixa(syst, l):
    ...
    salt = np.random.randint(2000000) # Seed
    t = time.time()
    f = open("G_m"+str(Nf)+"_n"+str(L)+"w477_R"+str(l)+".dat","w")
    g = open("S_m"+str(Nf)+"_n"+str(L)+"w477_R"+str(l)+".dat","w")
    for E in energies:
        smatrix = kwant.smatrix(syst, energy=E, params=dict(salt=salt, W=W*sigma_0))
        G_10    = smatrix.transmission(1,0)
...
def main():
    ...
    for l in range(0,400,1):
        ...
        shutil.move(f"mv G_m0_n53w477_R{l}.dat", "./G_m0_beta4")
        shutil.move(f"mv S_m0_n53w477_R{l}.dat", "./S_m0_beta4")
