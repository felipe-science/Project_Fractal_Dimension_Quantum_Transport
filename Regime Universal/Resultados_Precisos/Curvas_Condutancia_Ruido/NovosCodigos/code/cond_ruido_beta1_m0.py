...
def run(syst, i):
    ...
    # Mover o arquivo para o diretório 'output'
    if not os.path.exists("output"):
        os.makedirs("output")
    os.rename(f"G{i}.dat", f"output/G{i}.dat")
...
def G_E_desordemfixa(syst, l):
    ...
    if not os.path.exists("./G_m0_beta1"):
        os.makedirs("./G_m0_beta1")
    if not os.path.exists("./S_m0_beta1"):
        os.makedirs("./S_m0_beta1")

    f = open(f"G_m{Nf}_n{L}w172_R{l}.dat", "w")
    g = open(f"S_m{Nf}_n{L}w172_R{l}.dat", "w")
    for E in energies:
        smatrix = kwant.smatrix(syst, energy=E, params=dict(salt=salt, W=W * sigma_0))
        G_10 = smatrix.transmission(1, 0)
    ...
    os.rename(f"G_m{Nf}_n{L}w172_R{l}.dat", f"./G_m0_beta1/G_m{Nf}_n{L}w172_R{l}.dat")
    os.rename(f"S_m{Nf}_n{L}w172_R{l}.dat", f"./S_m0_beta1/S_m{Nf}_n{L}w172_R{l}.dat")
