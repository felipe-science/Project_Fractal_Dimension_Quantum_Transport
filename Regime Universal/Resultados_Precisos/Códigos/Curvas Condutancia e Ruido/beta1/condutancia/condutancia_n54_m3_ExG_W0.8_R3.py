...
def G_E_desordemfixa(syst):
    ...
    salt = np.random.randint(2000000) # Seed
    t = time.time()
    f = open("G_m"+str(Nf)+"_n"+str(L)+"w08_R3.dat","w")
    for E in energies:
        smatrix = kwant.smatrix(syst, energy=E, params=dict(salt=salt, W=W*sigma_0))
        G_10    = smatrix.transmission(1,0)
...
def main():
    ...
    #kwant.plot(syst)
    G_E_desordemfixa(syst)

    
if __name__ == '__main__':
    main()

#os.system("mv G_m3_n53w08_R1.dat    /content/drive/MyDrive/Pesquisa/PesquisaMestrado/Project_Disorder/Output_Condutancia_Ruido_Curvas/Beta1/m3/Condutancia/")