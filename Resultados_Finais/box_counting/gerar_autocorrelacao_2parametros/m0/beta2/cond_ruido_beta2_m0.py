...
def G_E_desordemfixa(syst, l):
    ...
    for E in energies:
        ...
        condutan.append(smatrix.transmission(1, 0))
        shotnois.append(kwant.physics.two_terminal_shotnoise(smatrix))

    # Salva os dados em arquivos .dat de uma só vez
    np.savetxt(f"G{l}.dat", np.column_stack((energies, condutan)), fmt="%.6e")
    np.savetxt(f"S{l}.dat", np.column_stack((energies, shotnois)), fmt="%.6e")
