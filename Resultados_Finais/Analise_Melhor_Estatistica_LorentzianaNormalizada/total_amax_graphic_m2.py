...
#m = 2
G_m2_beta1_curve = np.loadtxt('m2/G_beta1_m0/amax_curve.dat', float)
G_m2_beta2_curve = np.loadtxt('m2/G_beta2_m0/amax_curve.dat', float)
G_m2_beta4_curve = np.loadtxt('m2/G_beta4_m2/amax_curve.dat', float)
G_m2_beta1_point = np.loadtxt('m2/G_beta1_m0/amax_point.dat', float)
G_m2_beta2_point = np.loadtxt('m2/G_beta2_m0/amax_point.dat', float)
...
xc = G_m2_beta1_curve[:,0]/list_gamma_beta1[2]
yc = G_m2_beta1_curve[:,1]
xp = G_m2_beta1_point[:,0]/list_gamma_beta1[2]
yp = G_m2_beta1_point[:,1]
mean = np.average(yc)

axs[0].plot(xc, yc, label=r'$g(\epsilon)$', color='red', zorder=1)
axs[0].scatter(xp, yp, color='black', zorder=2)
axs[0].set_title(r'$m = 2$', fontsize=35)
...
xc = G_m2_beta2_curve[:,0]/list_gamma_beta2[2]
yc = G_m2_beta2_curve[:,1]
xp = G_m2_beta2_point[:,0]/list_gamma_beta2[2]
yp = G_m2_beta2_point[:,1]
mean = np.average(yc)
...
xc = G_m2_beta4_curve[:,0]/list_gamma_beta4[2]
yc = G_m2_beta4_curve[:,1]
xp = G_m2_beta4_point[:,0]/list_gamma_beta4[2]
yp = G_m2_beta4_point[:,1]
mean = np.average(yc)
...
plt.savefig("total_amax_m2.png", dpi=200)
plt.show()