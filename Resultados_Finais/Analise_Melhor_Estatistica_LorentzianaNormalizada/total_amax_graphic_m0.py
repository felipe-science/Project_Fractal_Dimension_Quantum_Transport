...
fig, axs = plt.subplots(nrows=3, ncols=1, figsize=(12, 6), constrained_layout=False)  


# Adicionar o primeiro gráfico
xc = G_m0_beta1_curve[:,0]/list_gamma_beta1[0]
yc = G_m0_beta1_curve[:,1]
xp = G_m0_beta1_point[:,0]/list_gamma_beta1[0]
yp = G_m0_beta1_point[:,1]
mean = np.average(yc)

axs[0].plot(xc, yc, label=r'$g(\epsilon)$', color='red', zorder=1)
axs[0].scatter(xp, yp, color='black', zorder=2)
axs[0].set_title(r'$m = 0$', fontsize=35)
...
xc = G_m0_beta2_curve[:,0]/list_gamma_beta2[0]
yc = G_m0_beta2_curve[:,1]
xp = G_m0_beta2_point[:,0]/list_gamma_beta2[0]
yp = G_m0_beta2_point[:,1]
mean = np.average(yc)
...
xc = G_m0_beta4_curve[:,0]/list_gamma_beta4[0]
yc = G_m0_beta4_curve[:,1]
xp = G_m0_beta4_point[:,0]/list_gamma_beta4[0]
yp = G_m0_beta4_point[:,1]
mean = np.average(yc)
...
plt.savefig("total_amax_m0.png", dpi=200)
plt.show()