...
def SN_E(syst):
    ...
    t = time.time()
    f = open("SN_m"+str(Nf)+"_n"+str(L)+".dat","w")
    for E in energies:
        smatrix = kwant.smatrix(syst, energy=E)
    ...
    figure()
    plot(energies, shotnoise)
    title("Ruído de disparo n = 54   m = 3", fontsize='14')
    xlabel("Energia [t]", fontsize='13')
    ylabel("Ruído de Disparo", fontsize='13')
    tick_params(labelsize = 13)
    savefig(("shotnoise_sierpins_m%i_n%i.png")%(Nf,L))
    show()
